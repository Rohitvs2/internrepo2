...
function App() {
  ...
  const handleStartQuiz = () => {
    let seed = null;
    if (user && user.email) {
      seed = Array.from(user.email).reduce((acc, c) => acc + c.charCodeAt(0), 0);
    }
    setShuffledQuestions(shuffleQuestions(questions, seed));
    setQuizStarted(true);
  };
      ...
      <main className="container mx-auto px-4 py-6">
        {!quizStarted ? (
          ...
          <div className={`grid gap-6 transition-all duration-300 ${showChat && quizCompleted ? 'lg:grid-cols-3' : 'grid-cols-1'}`}>
            ...
            {showChat && quizCompleted && shuffledQuestions.length > 0 && (
              <div className="lg:col-span-1">
                <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 shadow-2xl overflow-hidden h-full min-h-[600px]">
                  ...
                  <div className="h-full bg-white/5 backdrop-blur-sm">
                    <Chatbot questions={shuffledQuestions} />
                  </div>
                </div>
              </div>
            )}
